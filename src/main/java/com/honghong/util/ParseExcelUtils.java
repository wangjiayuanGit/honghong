...
public class ParseExcelUtils {
     ...
     * @return
     */
    public static List<String> getHead(InputStream in, String fileName) throws IOException {
        Workbook workbook = getWorkBook(in, fileName);
    ...
    private static <T> List<T> readExcel(Class<T> classzz, Workbook workbook, List<ExcelHead> excelHeads) throws Exception {
        ...
        for (int sheetIndex = 0; sheetIndex < sheetNum; sheetIndex++) {
            ...
            for (int rowIndex = firstRowNum + 1; rowIndex <= lastRowNum; rowIndex++) {
                ...
                if (checkRowNull(dataRow, firstCellNum, lastCellNum)) {
                    continue;
                }
                for (int cellIndex = firstCellNum; cellIndex < lastCellNum; cellIndex++) {
                    Cell headCell = head.getCell(cellIndex);
     ...
     *
     * @param row          待检验行
     * @param firstCellNum 该行第一个单元格
     * @param lastCellNum  该行最后一个单元格
     * @return boolean
     */
    private static boolean checkRowNull(Row row, short firstCellNum, short lastCellNum) {
        int num = 0;
        for (int cellIndex = firstCellNum; cellIndex < lastCellNum; cellIndex++) {
        ...
        return num == lastCellNum - firstCellNum;
    }
    ...
    static class MethodUtils {
        ...
        static String setMethodName(String propertyName) {
