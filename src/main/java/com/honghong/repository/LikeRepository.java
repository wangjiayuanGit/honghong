...
public interface LikeRepository extends JpaRepository<LikeDO, Long> {
    ...
    @Query(value = "SELECT T.id AS topicId, T.content AS content, T.update_at AS updateAt," +
            ...
            "WHERE T.id IN ( SELECT id FROM tb_topic WHERE user_id = ?1 ) GROUP BY T.id, U.id ORDER BY T.update_at DESC LIMIT ?2 , ?3", nativeQuery = true)
    List<Map<String, Object>> findAllByUserId(Long userId, Integer page, Integer size);
